...
def register_compose_down(subparsers: argparse._SubParsersAction):
    def call(cleanup: bool, profile: list[str], **kwargs):
        helper = CommandHelper()
...
def register_generate_auth_token(subparsers: argparse._SubParsersAction):
    def call(length: int, **kwargs):
        helper = CommandHelper()
...
def register_generate_certificate_chain(subparsers: argparse._SubParsersAction):
    def call(**kwargs):
        helper = CommandHelper()
...
def register_generate_certificate_files(subparsers: argparse._SubParsersAction):
    def call(**kwargs):
        helper = CommandHelper()
...
def register_generate_django_secret_key(subparsers: argparse._SubParsersAction):
    def call(**kwargs):
        helper = CommandHelper()
...
def register_generate_secure_password(subparsers: argparse._SubParsersAction):
    def call(length: int, **kwargs):
        helper = CommandHelper()
...
def register_init_workspace(subparsers: argparse._SubParsersAction):
    def call(**kwargs):
        helper = CommandHelper()
...
def register_lint(subparsers: argparse._SubParsersAction):
    def call(**kwargs):
        helper = CommandHelper()
...
def register_randomize_env_secrets(subparsers: argparse._SubParsersAction):
    def call(**kwags):
        helper = CommandHelper()
...
def register_shell(subparsers: argparse._SubParsersAction):
    def call(container: str, **kwargs):
        helper = CommandHelper()

        helper.execute_cmd(
            f"{helper.build_compose_cmd()} exec {container} python manage.py shell_plus"
        )
...
def register_show_outdated(subparsers: argparse._SubParsersAction):
    def call(**kwargs):
        helper = CommandHelper()
...
def register_stack_deploy(subparsers: argparse._SubParsersAction):
    def call(build: bool, **kwargs):
        helper = CommandHelper()
        helper.prepare_environment()
...
def register_stack_rm(subparsers: argparse._SubParsersAction):
    def call(**kwargs):
        helper = CommandHelper()
...
def register_test(subparsers: argparse._SubParsersAction):
    def call(extra_args: list[str], **kwargs):
        helper = CommandHelper()
...
def register_try_github_actions(subparsers: argparse._SubParsersAction):
    def call(**kwargs):
        helper = CommandHelper()
...
def register_upgrade_postgres_volume(subparsers: argparse._SubParsersAction):
    def call(version: str, **kwargs):
        helper = CommandHelper()
